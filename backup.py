...
def calls(actuator, UDP):
    ...
    while True:     #Definindo e estabelecendo comunicação com o servidor
        ...
        if actualUser==False:  #Caso seja a primeira iteração do usuário, pede para se declarar
            ...
            threading.Thread(target=list, args=()).start()
            message = input()
...
def run():
    actualUser = False
    threading.Thread(target=calls, args=(actualUser,UDP)).start()
    threading.Thread(target=listen, args=(UDP, )).start()
